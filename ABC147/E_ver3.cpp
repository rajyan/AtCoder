...
//	vector<bitset<offset * 2 + 1>> dp(W + 1);
//	dp[0][offset] = 1;
//	for (int i = 0; i < H; i++) {
//
//		for (int j = 0; j < W; j++) {
...
//					dp[j + 1][k] = dp[j + 1][k] | dp[j][k + A[i][j] - B[i][j]];
//				}
//				if (0 <= k - A[i][j] + B[i][j] && k - A[i][j] + B[i][j] <= 2 * offset) {
//					dp[j + 1][k] = dp[j + 1][k] | dp[j][k - A[i][j] + B[i][j]];
//				}
...
//
//		if (i == H - 1) break;
//		vector<bitset<offset * 2 + 1>> tmp(W + 1);
//		for (int j = 1; j <= W; j++) {
//			for (int k = 0; k <= 2 * offset; k++) {
//				if (0 <= k + A[i + 1][j - 1] - B[i + 1][j - 1] && k + A[i + 1][j - 1] - B[i + 1][j - 1] <= 2 * offset) {
//					tmp[j][k] = tmp[j][k] | dp[j][k + A[i + 1][j - 1] - B[i + 1][j - 1]];
//				}
//				if (0 <= k - A[i + 1][j - 1] + B[i + 1][j - 1] && k - A[i + 1][j - 1] + B[i + 1][j - 1] <= 2 * offset) {
//					tmp[j][k] = tmp[j][k] | dp[j][k - A[i + 1][j - 1] + B[i + 1][j - 1]];
//				}
...
//		if (dp[W][i]) chmin(ans, abs(i - offset));
//	}
//
//	cout << ans << "\n";
//
